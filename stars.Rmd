...
We saw an example of `st_rasterize` above, which rasterizes polygon data to `stars` rasters. `st_as_sf` does the reverse; 
...
Operations on proxy objects are **lazy**, that means that they are postponed up to the moment that grid cells are needed (i.e. to plot, or to write). For instance in the following sequence,

```{r}
ndvi = function(x) (x[4]-x[1])/(x[4]+x[1])
p.ndvi = st_apply(p, c("x", "y"), ndvi)
plot(p.ndvi)
```

the actual order of operations is reversed:

* when `plot(p.ndvi)` is called, nothing has been computed yet
* `plot.stars_proxy` evaluates what needs to be done; since many more pixels are present in the image then there are available on the plotting device, and since `ndvi` is applied pixel-wise, it only has to 
    * read the pixels at screen resolution
	* compute ndvi for those pixels
* this leads to a speed-up from 10 minutes (for all available pixels) to a few seconds (for visible pixels)
